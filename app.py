...
from utils.formatting import pct, risk_label, usd
...
st.markdown(
    """
<style>
.block-container {padding: .65rem 1.0rem 1rem 1.0rem; max-width: 1160px;}
h1 {font-size: 1.2rem !important; margin: 0 0 .35rem 0 !important; letter-spacing: -0.03em;}
h2 {font-size: .98rem !important; margin: .5rem 0 .25rem 0 !important;}
h3 {font-size: .88rem !important; margin: .4rem 0 .2rem 0 !important;}
...
.action-card strong {font-size:.85rem;}
.green {color:#22c55e;} .red {color:#ef4444;} .muted {color:#9ca3af;} .blue {color:#60a5fa;}
.status-pill {display:inline-block; border-radius:999px; padding:3px 8px; font-size:.72rem; background:#0f172a; border:1px solid #1f2937; color:#9ca3af; margin-left:6px;}
...
def recent_transaction_cards(limit=5):
    ...
    for _, row in tx.iterrows():
        ...
        label = f"{row['action']} {row['ticker']}"
        detail = f"{row['shares']:g} @ {usd(row['price'])} · {usd(amount)} · {row['date']}"
        st.markdown(
            f"""
<div class="action-card">
...
if nav == "Dashboard":
    ...
    with top_left:
        last_sync = get_setting("last_price_refresh", "Never")
        status = get_setting("last_price_status", "Not refreshed")
        st.caption(f"Compact portfolio dashboard  ·  Last price sync: {last_sync}")
        st.markdown(f'<span class="status-pill">{status}</span>', unsafe_allow_html=True)
    with top_right:
        if st.button("Refresh", use_container_width=True):
            result = refresh_prices()
            st.toast(f"Prices refreshed: {result['updated']} updated, {result['fallback']} fallback")
            st.rerun()
    kpi_row()
